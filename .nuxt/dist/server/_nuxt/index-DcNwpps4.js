import { defineComponent, ref, mergeProps, useSSRContext, unref, computed } from "vue";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isAuthenticated.value && workerEnabled.value) {
        ...
        if (!testModeCountdown.value) {
          _push(`<button type="button"${ssrIncludeBooleanAttr(frequencySaving.value) ? " disabled" : ""} class="px-4 py-2 text-sm bg-amber-600/80 hover:bg-amber-600 text-white rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed"> Test (1 min) </button>`);
        } else {
