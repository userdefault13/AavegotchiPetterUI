...
import { verifyMessage, createPublicClient, http, parseAbi } from 'file:///Users/juliuswong/Dev/AavegotchiPetterUI/node_modules/viem/_esm/index.js';
...
const assets = {
  "/index.mjs": {
    ...
    "etag": "\"18980-yX+vpg7DqV8UiTuGqpYJlCjFnuY\"",
    "mtime": "2026-02-17T07:43:15.442Z",
    "size": 100736,
    "path": "index.mjs"
  },
  "/index.mjs.map": {
    "type": "application/json",
    "etag": "\"57a79-P2LFDdHBMNd494Bt4XfyLMnTHsI\"",
    "mtime": "2026-02-17T07:43:15.442Z",
    "size": 359033,
    "path": "index.mjs.map"
  }
...
const _lazy_ZgUbvc = () => Promise.resolve().then(function () { return status_get$3; });
...
const _lazy_LLNynj = () => Promise.resolve().then(function () { return status_get$1; });
const _lazy_KSwdmL = () => Promise.resolve().then(function () { return clear_post$1; });
...
const handlers = [
  ...
  { route: '/api/delegation/status', handler: _lazy_LLNynj, lazy: true, middleware: false, method: "get" },
...
function isAddressAllowed(address, allowedAddress) {
  const allowed = (allowedAddress || DEFAULT_ALLOWED).toLowerCase();
  return address.toLowerCase() === allowed;
}
async function verifySignature(address, message, signature, allowedAddress) {
  try {
...
function checkAuth(event, allowedAddress) {
...
const check_get = defineEventHandler$1(async (event) => {
...
const me_get = defineEventHandler$1((event) => {
  if (!checkAuth(event)) {
    throw createError$1({
      statusCode: 401,
...
const verify_post = defineEventHandler$1(async (event) => {
  const config = useRuntimeConfig();
